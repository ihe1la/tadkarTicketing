...
```powershell
docker build -f docker/api.Dockerfile --target prod -t tadkar-api .
docker build -f docker/web.Dockerfile --target prod -t tadkar-web .
```

The API image serves on `http://localhost:3000`, and the web image serves on `http://localhost`.
